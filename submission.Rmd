...
At first, we fit the aforementioned models with the previously described basic pre-motion features as all of these features are derived at the time of line-set. These very basic models serve as baseline models that allows to measure the effect of pre-snap motion (features) in the following.
...
The two figures above show the results from our 50 repetitions of the experiment. The first figure illustrates the classification accuracy, computed using a threshold of 50\%. Since this value is somewhat arbitrary and our dataset is slightly imbalanced ($\approx$ 75\% zone coverage plays and $\approx$ 25\% man coverage plays), accuracy is a suboptimal metric for evaluating the results. Hence, we also present the AUC values of the three models (see Appendix for a discussion on evaluation metrics) in the second figure. 
...
On the other hand, the pre-motion model clearly performs worst. Once motion information is incorporated, we observe a substantial improvement in performance.
...
Comparing predictions from pre-motion and post-motion models provides insights into an offense's ability to use motion effectively to identify defensive coverage schemes. Similarly, future analysis could also relate to evaluating a defense's skill in disguising their coverage and confusing the offense during pre-snap motion.
