######################################################################################
######################################################################################
#### BDB 2025 preprocessing: team analyses
######################################################################################
######################################################################################
...
full_data = readRDS("data/zvm_motion_mod_data.rds")
...
CIN_plays <- model_data_full |> 
  filter(possessionTeam == "CIN") |>
  select(-week,-possessionTeam)

prop.table(table(CIN_plays$pff_manZone_num))

NCIN_plays <- model_data_full |> 
  filter(possessionTeam != "CIN") |>
  select(-week,-possessionTeam)

prop.table(table(NCIN_plays$pff_manZone_num))

seedu <- 42

CIN_xy_npo <- zvm_models(NCIN_plays,CIN_plays,
                         ml_type = "xgboost",
...
CIN_xy_po <- zvm_models(NCIN_plays,CIN_plays,
                        ml_type = "xgboost",
                        excl_vars = c(
                          names(model_data)[grep("dist_fb",names(model_data))],
                          names(model_data)[grep("def_o_qb_diff",names(model_data))]),
...
CIN_res_tab <- data.frame(zvm_num = CIN_plays$pff_manZone_num,
                      zvm = ifelse(CIN_plays$pff_manZone_num == 1, "Man","Zone"),
                      pre_man_prob = CIN_xy_npo$preds,
                      post_man_prob = CIN_xy_po$preds,
                      pre_zone_prob = 1-CIN_xy_npo$preds,
                      post_zone_prob = 1-CIN_xy_po$preds
                      )

CIN_zone <- CIN_res_tab |> 
  filter(zvm == "Zone") |>
  select(pre_zone_prob,post_zone_prob) |>
  mutate(improvement = ifelse(pre_zone_prob < post_zone_prob,1,0))

table(CIN_zone$improvement)
...
table(CIN_man$improvement)


##  SF model -----------------------------------------


SF_plays <- model_data_full |> 
  filter(possessionTeam == "SF") |>
  select(-week,-possessionTeam)

prop.table(table(SF_plays$pff_manZone_num))

NSF_plays <- model_data_full |> 
  filter(possessionTeam != "SF") |>
  select(-week,-possessionTeam)

prop.table(table(NSF_plays$pff_manZone_num))

seedu <- 42

SF_xy_npo <- zvm_models(NSF_plays,SF_plays,
                         ml_type = "xgboost",
                         excl_vars = c("average","sum","nr_player_changes","average_ent",
                                       names(model_data)[grep("dist_fb",names(model_data))],
                                       names(model_data)[grep("def_o_qb_diff",names(model_data))],
...
SF_xy_po <- zvm_models(NSF_plays,SF_plays,
                        ml_type = "xgboost",
                        excl_vars = c(
                          names(model_data)[grep("dist_fb",names(model_data))],
                          names(model_data)[grep("def_o_qb_diff",names(model_data))]),
...
SF_res_tab <- data.frame(zvm_num = SF_plays$pff_manZone_num,
                          zvm = ifelse(SF_plays$pff_manZone_num == 1, "Man","Zone"),
                          pre_man_prob = SF_xy_npo$preds,
                          post_man_prob = SF_xy_po$preds,
                          pre_zone_prob = 1-SF_xy_npo$preds,
                          post_zone_prob = 1-SF_xy_po$preds
)
...
saveRDS(all_team_zvm,"data/teams_zvm_030125.rds")
...
# })
# 
# diff_zone <- lapply(seq_along(diffslz), \(i){
#   data.frame(diff = diffslz[[i]],team = names(diffslz)[[i]])
# }) |>
#   bind_rows()
# 
# diffslm <- lapply(all_team_zvm,\(x){
#   mt <- x$man_table |>
#     mutate(diff = post_man_prob-pre_man_prob)
#   
#   return(c(mt$diff))
# })
# 
# diff_man <- lapply(seq_along(diffslm), \(i){
#   data.frame(diff = diffslm[[i]],team = names(diffslm)[[i]])
# }) |>
#   bind_rows()
...
#   xlab("") +
#   labs(title = "**Distribution of differences in pre and post motion probability**",
#        subtitle = "*NFL Teams*") +
#   theme(axis.text.x = element_nfl_logo(size = 0.75))
# 
# diff_man |> 
...
saveRDS(team_info,"team_tab.rds")
